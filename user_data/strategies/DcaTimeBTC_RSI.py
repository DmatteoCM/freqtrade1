...
class DcaTimeBTC_RSI(IStrategy):
    """
    Variante DcaTimeBTC: entry a market con solo filtro RSI 1h (nessun BB lower).
    Entry se RSI 1h < rsi_buy — evita il rientro in fase di ipercomprato estremo.
    ...
    rsi_buy   = IntParameter(30, 80,  default=70,  space="buy", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if rsi_col not in dataframe.columns:
            return dataframe
        # Entry a market con solo filtro RSI: evita rientro in ipercomprato
        condition = (dataframe[rsi_col] < self.rsi_buy.value) & (dataframe["volume"] > 0)
