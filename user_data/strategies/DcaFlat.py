...
class DcaFlat(IStrategy):
    """
    DcaFlat — tranche flat 6 USDC, tranche illimitate, nessuna uscita forzata
    - Ogni tranche: 6 USDC fissi (nessun incremento progressivo)
    - Nessun limite di tranche (max_entry_position_adjustment = -1)
    - Nessun time_exit: il trade rimane aperto finché non scatta il trailing stop
    - Uscita: trailing stop attivato da BB superiore 1h + RSI 1h (stesso di DcaProgressive)
    """
    ...
    max_entry_position_adjustment = -1  # illimitato
    ...
    rsi_t1    = IntParameter(25, 45, default=30, space="buy",  optimize=True)
    rsi_t2    = IntParameter(20, 35, default=35, space="buy",  optimize=True)
    rsi_t3    = IntParameter(15, 28, default=27, space="buy",  optimize=True)
    adx_thresh = IntParameter(15, 35, default=26, space="buy", optimize=True)
    vol_mult  = DecimalParameter(1.0, 2.5, default=1.1, decimals=1, space="buy", optimize=True)
    rsi_sell     = IntParameter(55, 80, default=74, space="sell", optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        entry = ((above & momentum) | (~above & accumulo)) & (dataframe["volume"] > 0)
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        rsi_thresh = self._rsi_thresh(nr + 1)
        ...
        stake = min(self.tranche_size, max_stake)
